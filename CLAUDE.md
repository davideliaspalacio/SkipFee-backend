...
- **Cron** (header `x-cron-secret`): `inactivity-check`, `expire-drafts`, `survey-dispatch`.
...
- **`settings`** — **fila única `id=1`**. Horarios (`hours` jsonb por día, `open/close_hour`, `open_days`), `base_delivery_fee`, `orders_paused`, `reminder_minutes`, ventana de entregados, config post-venta (`postventa_enabled`, `survey_delay_minutes`, `survey_min_days`, `review_gift_product_id`), `review_categories`(jsonb).
...
- **Kapso:** `lib/kapso/`. Entrada por `webhooks/kapso` (verifica `X-Webhook-Signature` HMAC-SHA256 con `KAPSO_WEBHOOK_SECRET`, dedup por `webhook_events`). Salida `sendText/sendImage/sendButtons/sendList`.
...
- **Single-tenant:** todo asume un negocio (ver `../CLAUDE.md` §9).
