...
export async function GET(_request: NextRequest, ctx: { params: Promise<{ orderId: string }> }) {
  ...
  // Catálogo + zonas + settings + promos activas (todo en paralelo).
  // Las promos las recalculamos en cada GET para que el cart devuelto siempre
  // refleje la promo vigente AHORA (puede haber cambiado desde el último PUT).
  const [{ data: products }, { data: zones }, { data: settings }, { data: promotions }] = await Promise.all([
