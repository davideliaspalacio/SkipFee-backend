...
/**
 * GET /api/<companyId>/reports/summary?period=7d|30d|90d
 * Devuelve agregaciones para la pantalla Reportes de LA empresa.
 */
export const GET = withTenant(async (request, ctx) => {
  ...
  const [{ data: currentOrders }, { data: prevOrders }, { data: zonesData }] = await Promise.all([
    ...
    sb.from('zones').select('id, name, tarifa, recargo').eq('company_id', companyId),
  ]);
