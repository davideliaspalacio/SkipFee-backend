...
/**
 * GET /api/<companyId>/promotions
 * Lista promociones (activas e inactivas) de LA empresa, ordenadas por
 * kind+nombre. Por defecto excluye las archivadas; con `?all=1` las incluye
 * (lo usa el panel admin para mostrar la sección de archivadas). El storefront
 * usa /api/<companyId>/promotions/active (filtrado + hidratado con productos).
 */
...
/**
 * POST /api/<companyId>/promotions
 * Crea una promoción para LA empresa. Valida que la combinación kind+config sea
 * coherente (un kind='weekday' sin weekdays no tiene sentido).
 */
