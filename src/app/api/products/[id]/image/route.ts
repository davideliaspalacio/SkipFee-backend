 ...
 *
 * Naming: `<product_id>/<timestamp>.<ext>` — mantiene histórico cuando el
 * cliente reemplaza la imagen (storage es barato, se limpia con cron a
 * futuro si crece mucho).
 *
 * Validaciones:
 *   - MIME debe empezar con `image/`
 *   - tamaño ≤ 5 MB
 *   - producto debe existir (devuelve 404 si no)
 */
...
export async function POST(
  ...
  // 1. Verificar que el producto existe (no perder el upload contra un id inválido).
  ...
  // 3. Subir al bucket. `<id>/<timestamp>.<ext>`.
